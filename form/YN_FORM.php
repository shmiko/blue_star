	...
	<div id="mainForm">
		...
		<BR/>
			...
			<form method=post enctype=multipart/form-data action="YN_XR_QUERY_2.php" method="POST" ><ul class=mainForm id="mainForm_1">
				...
				<label class="formFieldQuestion">Report<a class=info href=#><img src=imgs/tip_small.png border=0><span class=infobox>Enter the Report</span></a></label><input name="report" type="text" size="20" maxlength="20" value="49"></li>
				<?php
					// Create connection to Oracle
					$c = oci_connect("PWIN175", "PWIN175", "ORABLUESTAR");
					$query = 'SELECT RM_CUST FROM PWIN175.RM WHERE RM_PARENT IS NULL';
					$s = oci_parse($c, $query );
					oci_execute($s);
					print "<select name='report'>";
					  while ($row = oci_fetch_array($s, OCI_RETURN_NULLS+OCI_ASSOC)) 
					{
					   print "<option value='' >".htmlspecialchars($oci_fetch_array["report"])."</option>";
					}
					print "</select>";
				?>
				<li class="mainForm">
